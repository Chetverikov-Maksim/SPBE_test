...
## ⚠️ ВАЖНОЕ ПРИМЕЧАНИЕ - РЕШЕНИЕ РЕАЛИЗОВАНО! ✅

**ПРОБЛЕМА РЕШЕНА**: Сайт СПБ Биржи блокирует автоматические запросы (HTTP 403 Forbidden), но парсер теперь включает **поддержку Selenium** для обхода защиты от ботов!

**Для запуска парсера**:
1. Установите Chrome/Chromium на вашей машине
2. В файле `spbe_parser/config.py` установите `USE_SELENIUM = True`
3. Запустите парсер - он автоматически скачает ChromeDriver и обойдет блокировку

**Альтернативные решения**: См. [PROBLEM_AND_SOLUTIONS.md](PROBLEM_AND_SOLUTIONS.md)
...
### Настройка Selenium (РЕКОМЕНДУЕТСЯ)

Для обхода блокировки сайта СПБ Биржи:
...
**2. Включите Selenium в конфигурации:**

Откройте файл `spbe_parser/config.py` и измените:
```python
USE_SELENIUM = True  # Было: False
```

**3. Готово!** Парсер автоматически скачает ChromeDriver при первом запуске.

**Примечание**: Selenium работает медленнее обычных HTTP запросов (2-5 сек на страницу), но успешно обходит защиту от ботов.
